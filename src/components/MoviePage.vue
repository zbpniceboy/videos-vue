<template>
	<el-main class="pageWidth">
		...
		<div class="tvpage-type">
			...
			<ul :id="item.name">
				...
				<template v-for="(ite,index) in catlist.area" :key="'A'+index" >
		...
		<div class="main-list">
		<el-row :gutter="10" v-if="moviedata.Movielist && !isnull">
			<template v-for="(item,index) in moviedata.Movielist" :key="item+index">
			<el-col :span="5"  v-if="!isNaN(index)">
				...
				<el-card :body-style="{ padding: '0px'}" shadow="hover">
					...
					<div class="card-text-box">
						...
						<div class="card-desc" v-if="item.comment">{{item.comment}}</div>
						<div class="card-desc" v-else>主演：{{item.actor}}</div>
					</div>
				</el-card>
				</div>
				</router-link>
	...
	<el-pagination v-if="moviedata.Movielist && !isnull" 
	...
	setup(props){
		...
		const compute = computed(() => {
			...
			if(props.moviedata.length != 0){
				pagebar = Math.ceil(props.moviedata.Movielist['total'] / props.moviedata.Movielist['curlist'])  * 10 || '200';
				if(pagebar > 200){
					pagebar = 200
				}
			}
		...
		watchEffect(() => {
				...
				if(JSON.stringify(useRouter) != "{}" && proxy.$route.name == 'movie'){
					datas.types = {
						rank:datas.rank,
						cat:datas.cat,
						area:datas.area,
