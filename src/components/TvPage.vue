<template>
	...
	<template #default>
		...
		<div class="main-list">
		<el-row :gutter="10" v-if="tvdata.tvlist && !isnull">
			<template v-for="(list,index) in tvdata.tvlist" :key="list+index">
			<el-col :span="5"  v-if="!isNaN(index)">
				<router-link :to="{ name: 'player', query:{mtype:'tv',id:list.id}}" >
				<div class="grid-content bg-purple">
				<el-card :body-style="{ padding: '0px'}" shadow="hover">
