<template>
	<Header :swiper = "false" />
	<div class="pusher">
	<Slotmainlist :slotdata="datas.moviedata.Movielist" :isnull="datas.isnull" :loading ="datas.isLoading">
...
export default defineComponent({
	...
	setup() {
		...
		const compute = computed(() => {
			var pagebar = '';
			if(datas.moviedata.length != 0){
				pagebar = Math.ceil(datas.moviedata.Movielist['total'] / datas.moviedata.Movielist['curlist'])  * 10 || '200';
				if(pagebar > 200){
					pagebar = 200
				}
			}
